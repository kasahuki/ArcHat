<template>
  <div class="todo-list-root" @click.self="clearSelection">
    ...
    <div class="todo-progress-row">
      <svg class="progress-pie" width="48" height="48" viewBox="0 0 40 40">
        <circle cx="20" cy="20" r="18" fill="#f5f6fc" />
        <circle
          cx="20" cy="20" r="18"
...
function editAndClear(todo) {
  selectedIds.value = [];
  startEdit(todo);
}
function saveEdit(todo) {
  if (editingId.value === todo.id && editText.value.trim() && (editText.value !== todo.text || editCategory.value !== todo.category)) {
...
const progressOffset = computed(() => circumference * (1 - progressPercent.value / 100));
const progressColor = computed(() => progressPercent.value === 100 ? '#4cd964' : '#5371f7');

function handleReorderTodo(newList) {
  todoStore.todos = newList;
  todoStore.save && todoStore.save(); // 如果 store 有 save 方法则调用
}
</script>
<style scoped>
.todo-list-root {
